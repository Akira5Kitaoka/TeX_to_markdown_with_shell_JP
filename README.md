...
できること：
- 定理 / 補題 / 定義などの環境を blockquote 形式に変換（PDF と同じ番号付け）
- `\ref / \eqref / \cref / \Cref` を Markdown 内リンクに変換
- `\cite / \citet / \citep / \cites` を `[著者 西暦](#ref-key)` 形式のリンクに変換
- `\href{url}{text}` を `[text](url)` に変換（bib の `note` フィールド内も対応）
- bib ファイルから参考文献節を自動生成（`note` フィールドも反映）
- `\appendix` 以降の見出し / 数式 / 定理番号に「付録」プレフィックスを付与
- `--Qiita` フラグで Qiita 互換出力にも対応
...
`--Qiita` フラグを付けると Qiita のサニタイザ／KaTeX 制約に合わせた出力になります．

```bash
bash tex_to_md.sh hogehoge.tex --Qiita
```

通常モードとの差分：

| 項目 | 通常モード (GitHub Pages + MathJax) | `--Qiita` モード |
|:---|:---|:---|
| 数式中の `\label{...}` | `\tag{N}\label{key}` を出力 | `\tag{N}` のみ (KaTeX は `\label` 非対応) |
| 数式 / 定理 / 見出しの `<a id="...">` | 出力する | 出力しない (Qiita が `id` 属性を除去するため) |
| `proof` 末尾の QED | `<div style="text-align: right">$\square$</div>` | 本文末尾に `Q.E.D.` を inline で付加 |
| `\begin{align}...\\...\end{align}` 内の `\\` | そのまま | `\\\\` に倍化 (markdown エスケープ対策) |
| `[text](#anchor)` の内部 fragment リンク | リンクとして残す | リンクを外し `[text]` のみに (jump が効かないため) |
| 外部 URL (`[text](https://...)`) | そのまま | そのまま (画像 `![alt](url)` も保持) |
| `<a id="key">XXX</a>` | そのまま | タグを外して `XXX` のみ残す |
...
### 出力フォーマット
- GitHub Pages (MathJax 対応) での表示を想定（`--Qiita` 指定時は Qiita 用に整形）．
    - 数式は MathJax 互換 (`$ ... $` / `$$ ... $$`) で出力．
    - 定理系は blockquote (`> **定理 N**`) として出力．label が無くても PDF と同じ番号が振られる（preamble の `aliascnt` 設定に従い `theorem` / `lemma` / `definition` / `remark` … はカウンタを共有）．
    - 数式アンカー / 参考文献アンカーは `<a id="...">` で埋め込み，`\ref` 等から内部リンクで参照可能．
    - 色指定は `<font color=#...>` として出力．
    - タイトル / 著者は HTML コメント (`<!-- ... -->`) として先頭に挿入．
...
| URL リンク | `\href{url}{text}` / `\url{url}` | それぞれ `[text](url)` / `<url>` に変換．bib の `note` フィールド内も対応 |
...
| タイトル | `\title` / `\author` / `\maketitle` | 先頭に HTML コメント (`<!-- ... -->`) として挿入 |

### bib ファイルの扱い
- カレントディレクトリの `*.bib` をすべて拾うのではなく，TeX ファイル中の `\bibliography{...}` で指定された bib のみを使用する．
- 引用された (`\cite` 等で参照された) キーのみ参考文献節に出力する．
- `author` / `title` / `year` / `journal` (or `booktitle` / `institution` / `publisher`) / `volume` / `number` / `pages` / `note` を順にカンマ区切りで連結．
- `note` フィールド内の `\href{url}{text}` は `[text](url)` に展開される．
